...
namespace MVersionDeviceDemo
{
    class IKDeviceCXP : IKDevice
    {
        ...
        public override bool openDevice(int nDevIndex, int nBoardIndex)
        {
            ...
            res = IKapCLib.ItkManGetCXPDeviceInfo((uint)m_nDevIndex, ref camInfo);
            if (!Check(res))
            {
                System.Diagnostics.Debug.WriteLine("Camera error:Get cxp camera information failed");
                return false;
            }
            ...
            if(camInfo.Reserved != "")
                boaInfo.Reserved = System.Text.Encoding.UTF8.GetBytes(camInfo.Reserved);
            boaInfo.SlaveCount = camInfo.SlaveCount;
            boaInfo.SlavePort = camInfo.SlavePort;
            m_pBoard = IKapBoard.IKapOpenCXP((uint)BoardType.IKBoardPCIE, (uint)nBoardIndex, boaInfo);
        ...
        public override bool startGrab(int nCount)
        {
            ...
            m_nCurFrameIndex = 0;
            IKapCLib.ItkDevExecuteCommand(m_pDev, "AcquisitionStart");
